...
class airTrafficControl extends Thread{
    ...
    public void getDetails(int ch){
        ...
        String nul="";
            ...
            fweight = sc.nextInt();
            if (fweight > 12 || fweight > 20 || fweight > 40 || fweight > 100) {
                System.out.println(fweight+"Heavy weight cannot be processed......Enter a weight below 100");
                return ;
            }
    ...
    public void runwayAssign(){
        ...
        for(int i=0;i<runWays.size();i++)
        {
            if(compTime<=runWays.get(i).time && runWays.get(i).status){
                ...
                r.start();
                try{Thread.sleep(500);} catch(Exception e){}//for dont interrupt two thread
                break;
            }
        }
